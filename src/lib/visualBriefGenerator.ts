...
const fallbackStopTerms = new Set(
  tokenize(
    [
      ...
      "melankoli öfke şefkat yorgunluk absürtlük açıklık arzu umut absürt duygusal duygu",
...
function similarityTerms(value: string): string[] {
  return Array.from(new Set(tokenize(value).filter((term) => term.length >= 4)));
}
...
function visualBriefTooSimilar(candidate: VisualBrief, recent: RecentVisualBrief[]): boolean {
  ...
  return recent.some((brief) => {
    ...
    const subjectOverlap = jaccard(similarityTerms(candidate.visual_subject), similarityTerms(brief.visual_subject));
    const fullOverlap = jaccard(candidateTerms, similarityTerms(textForSimilarity(brief)));
    return subjectOverlap >= 0.5 || fullOverlap >= 0.34;
  });
}
...
function localFallbackBrief(input: GenerateVisualBriefWithLLMInput, reason: string): VisualBriefGenerationResult {
  ...
  const selected = seededMany(terms.length > 0 ? terms : dominantMood(input.mood), `${input.date}:visual-brief-fallback`, 7);
  const motif = seededMany(transformedMotifs, `${input.date}:visual-brief-motif`, 1)[0] ?? "kalıntı";
  const subjectSeed = selected.slice(0, 3).join(", ");
  const anchor = selected.slice(3, 6).join(", ") || truncateWords(input.mood_sentence, 8);
  const subject = subjectSeed
    ? `${subjectSeed} izinden türeyen ${motif}`
    : `bugünün iç tortusundan türeyen ${motif}`;
  return {
    brief: {
      visual_subject: subject,
      content_anchor: anchor || dailyMemorySentence(input.daily_life),
      composition_logic: "tek merkeze kilitlenmeyen, seçilen kalıntıyı kenarlara sızdıran kırık bir düzen",
      material_logic: "lekeli yüzey, bulanık gölge, yıpranmış dijital doku ve nesneleşmeyen iç katman",
      movement_or_tension: "tanınır sahneye dönüşmeden belirme ile dağılma arasında kalan gerilim",
...
function buildVisualBriefPrompt(input: GenerateVisualBriefWithLLMInput, rejectedBriefs: VisualBrief[]): string {
  ...
  return [
    ...
    "visual_subject bir kategori adı, genel soyut atmosfer veya tekrar eden görev cümlesi olmasın; bugüne ait tekil bir görsel bahane gibi adlandır.",
